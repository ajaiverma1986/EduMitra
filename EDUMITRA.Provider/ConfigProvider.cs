...
namespace EDUMITRA.Provider
{
    public class ConfigProvider:BaseProvider
    {
        ...
        public async Task<SimpleResponse> AddPaymentAccounts(AddPaymentAccountMasterRequest request, IEDUMITRAServiceUser serviceUser)
        {
            SimpleResponse response = new SimpleResponse();
            response.Result = await _repository.AddPaymentAccounts(request, serviceUser);
            return response;
        }
        public async Task<SimpleResponse> AddNewTransactionslab(AddTxnslabRequest request, IEDUMITRAServiceUser serviceUser)
        {
            SimpleResponse response = new SimpleResponse();
            response.Result = await _repository.AddTransactionslab(request, serviceUser);
            return response;
        }
        public async Task<SimpleResponse> changesPaymentAccountsStatus(ChangePaymentAccStatusRequest request, IEDUMITRAServiceUser serviceUser)
        {
            SimpleResponse response = new SimpleResponse();
            response.Result = await _repository.changesPaymentAccountsStatus(request, serviceUser);
            return response;
        }
        public async Task<SimpleResponse> CreateNewApplication(CreateapplicationRequest request, IEDUMITRAServiceUser serviceUser)
        {
            SimpleResponse response = new SimpleResponse();
            if(request==null)
            {
